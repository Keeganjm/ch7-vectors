...
words <- c('crystal', 'quartz', 'emerald', 'diamond', 'coral', 'aquamarine', 'gold')
...
# "g" through "m"
g.m.words <- words[substring(words, 1, 1) >= 'g' & substring(words, 1, 1) <= 'm']
print(g.m.words)
...
WordBin <- function(words, l1, l2) {
  if (l1 < l2) {
    return(WordBin(words, l2, l1))
  }
  return(words[substring(words, 1, 1) <= l1 & substring(words, 1, 1) >= l2])
}


# Use your `WordBin` function to determine which of your words start with "e" 
# through "q"
print(WordBin(words, 'q', 'e'))
